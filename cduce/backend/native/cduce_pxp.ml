...
let of_string s =
  let enc, s =
    if String.length s >= 3 && String.sub s 0 3 = utf8_bom then
      (Some `Enc_utf8, String.sub s 3 (String.length s - 3))
    else (None, s)
  in
...
let of_file fn =
  ...
  let e =
    if buf = Bytes.unsafe_of_string utf8_bom then Some `Enc_utf8
    else (
      seek_in ic 0;
      None)
  in
...
let channel_of_id rid =
  ...
  in
  let enc, ch =
    if Url.is_url url then of_string (Url.load_url url) else of_file url
  in
...
let load_pxp handlers uri =
  try
    ...
    process_entity pxp_config (`Entry_document [ `Parse_xml_decl ]) mgr handlers
  with
  | exn -> Value.failwith' (Pxp_types.string_of_exn exn)

let use () =
  Load_xml.xml_parser := load_pxp pxp_handle_event;
...
let () = Cduce_config.register ~priority:~-1 "pxp" "PXP XML parser" use
