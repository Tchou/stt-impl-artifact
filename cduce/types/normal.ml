...
module Make (X1 : S) (X2 : S) = struct
  ...
  *)
  let rec add root t1 t2 = function
    ...
    | (s1, s2) :: rem ->
        ...
        if X1.is_empty i then add ((s1, s2) :: root) t1 t2 rem
          ...
          if not (X1.is_empty j) then add root j t2 rem
          else List.rev_append root rem

  let normal x = List.fold_left (fun accu (t1, t2) -> add [] t1 t2 accu) [] x
  ...
  let boolean_normal x = cleanup (List.fold_left line [] x)
  let boolean x = List.fold_left (fun accu x -> line [] x @ accu) [] x
  let pi1 l = List.fold_left (fun accu (t1, _) -> X1.cup accu t1) X1.empty l
  let pi2 l = List.fold_left (fun accu (_, t2) -> X2.cup accu t2) X2.empty l

  let pi2_restricted restr =
    List.fold_left
      (fun accu (t1, t2) ->
        if X1.is_empty (X1.cap t1 restr) then accu else X2.cup accu t2)
      X2.empty
end
