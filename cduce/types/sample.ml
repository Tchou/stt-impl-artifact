...
let rec get memo t =
  ...
  in
  let record (r, _, none) =
    let r = LabelMap.filter (fun _l (o, _t) -> not o) r in
    Types.record_fields (not none, LabelMap.map fields r)
  in
  let typ u =
    let u = Types.cap t u in
    if Types.is_empty u then raise Not_found else u
  in
  try try_seq typ [ Types.Int.any; Types.Atom.any; Types.Char.any ] with
  | Not_found -> (
      try try_seq pair (Types.Product.get t) with
      | Not_found -> (
          try try_seq xml (Types.Product.get ~kind:`XML t) with
          | Not_found -> (
              try
                let r = Types.Record.get t in
                let r =
                  List.sort (fun (_, _, n1) (_, _, n2) -> -compare n1 n2) r
                in
                try_seq record r
              with
              | Not_found -> (
                  try Types.Arrow.sample t with
...
let rec single memo t =
  ...
  let atom t = Types.Atom (AtomSet.single (Types.Atom.get t)) in
  let char t = Types.Char (CharSet.single (Types.Char.get t)) in
  let fields = function
    | true, _ -> assert false
    | false, t -> single memo t
  in
  let record = function
    | r, false, true ->
        let r =
