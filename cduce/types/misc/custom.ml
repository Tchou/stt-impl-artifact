...
module Dummy = struct
  let dump _ppf _ = failwith "dump not implemented"
  let check _ = failwith "check not implemented"
  let equal _t1 _t2 = failwith "equal not implemented"
  let hash _t = failwith "hash not implemented"
  let compare _t1 _t2 = failwith "compare not implemented"
end
...
module String : T with type t = string = struct
  ...
  let compare (s1 : string) (s2 : string) =
    ...
    if l1 != l2 then l2 - l1 else Stdlib.compare s1 s2

  let equal (s1 : string) (s2 : string) = s1 == s2 || s1 = s2
  let hash (s : string) = Hashtbl.hash s
end

module Bytes : T with type t = bytes = struct
  type t = bytes

  let dump fmt b = Format.pp_print_string fmt (Bytes.to_string b)
  let check _b = ()
  let compare (s1 : bytes) (s2 : bytes) = Stdlib.compare s1 s2
  let equal (s1 : bytes) (s2 : bytes) = s1 == s2 || s1 = s2
  let hash (s : bytes) = Hashtbl.hash s
end
...
module Array (X : T) = struct
  ...
  let compare a1 a2 =
    ...
    let c = std_compare l1 l2 in
    if c <> 0 then c else compare_elems a1 a2 0 l1
...
module Print = struct
  ...
  let pp_sep_space s ppf () = fprintf ppf s
  let pp_semi = pp_sep_space ";@ "
  let pp_comma = pp_sep_space ",@ "

  let pp_list pe ppf l =
    fprintf ppf "@[[";
    fprintf ppf "%a" (pp_print_list ~pp_sep:pp_semi pe) l;
    fprintf ppf "]@]"
