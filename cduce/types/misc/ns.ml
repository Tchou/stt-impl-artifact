...
let empty_table =
  mktbl Table.empty
    [
      ("", empty); ("xml", xml_ns); ("xmlns", xmlns_ns); (xmlns_ns_str, xmlns_ns);
    ]
...
let get_table table =
  Table.fold
    (fun prefix ns r ->
      let std =
        try Uri.equal (Table.find prefix empty_table) ns with
        | Not_found -> false
      in
      if std then r else (prefix, ns) :: r)
    table []
...
module Printer = struct
  ...
  let get_prefix p ns =
    try H.find p.ns_to_prefix ns with
    | Not_found ->
        let r = ref (Hint []) in
  ...
  let is_really_free p pr =
    is_prefix_free p pr && not (List.exists (fun pr' -> U.equal pr pr') p.hints)
  ...
  let find_good_prefix p ns hint =
    try List.find (is_prefix_free p) hint with
    | Not_found -> (
        try List.find (is_really_free p) (Hashtbl.find_all global_hints ns) with
        | Not_found -> fresh_prefix p)

  let add_prefix p pr ns =
    if ns != empty || U.get_str pr <> "" then
      p.prefixes <- (pr, ns) :: p.prefixes

  let register_ns p ns =
    if ns == xml_ns || ns == xmlns_ns then ()
    else
      match get_prefix p ns with
      | { contents = Hint l } as r ->
...
module InternalPrinter = struct
  let p = ref (Printer.printer def_table)
  let set_table t = p := Printer.printer t
  ...
  let print_tag ppf q = Format.fprintf ppf "%s" (tag q)
  let print_attr ppf q = Format.fprintf ppf "%s" (attr q)
  let print_any_ns ppf ns = Format.fprintf ppf "%s" (any_ns ns)
end
...
let merge_tables t1 t2 = Table.fold add_prefix t2 t1

let dump_table ppf table =
  Table.iter
    (fun pr ns -> Format.fprintf ppf "%a=>\"%a\"@." U.print pr Uri.print ns)
    table

exception UnknownPrefix of U.t

let map_prefix table pr =
  try Table.find pr table with
  | Not_found -> raise (UnknownPrefix pr)
...
let att table ((pr, local), v) =
  (Label.mk ((if pr = "" then empty else map_prefix table (U.mk pr)), local), v)
  ...
  let rec aux (table : table) (attrs : ((string * U.t) * U.t) list) = function
    ...
    | (n, v) :: rest -> (
        ...
        | "xmlns", pr ->
            ...
            aux table attrs rest
        | x -> aux table ((x, U.mk v) :: attrs) rest)
  in
  aux table [] attrs

let process_start_tag_subst table tag attrs subst_hash =
  let real_ns ns = if H.mem subst_hash ns then H.find subst_hash ns else ns in
  let rec aux (table : table) (attrs : ((string * U.t) * U.t) list) = function
    | [] -> (table, map_tag table (U.mk tag), List.rev_map (att table) attrs)
    | ("xmlns", uri) :: rest ->
        let table = add_prefix empty_str (real_ns (Uri.mk (U.mk uri))) table in
        aux table attrs rest
    | (n, v) :: rest -> (
        match split_qname (U.mk n) with
        | "xmlns", pr ->
            let table = add_prefix pr (real_ns (Uri.mk (U.mk v))) table in
            aux table attrs rest
        | x -> aux table ((x, U.mk v) :: attrs) rest)
  in
  aux table [] attrs
