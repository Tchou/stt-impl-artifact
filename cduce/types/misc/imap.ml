...
let acreate n = Array.make n (Obj.repr n)
...
let elements (t : 'a t) : (int * 'a) list =
  if t == empty then []
    ...
    let rec aux accu i =
      if i > 0 then
        aux ((get_int t i, Obj.magic (get t (succ i))) :: accu) (i - 2)
      else accu
    in
    aux [] (get_len t - 2)
...
let map_elements f t =
  if t == empty then []
    ...
    let rec aux accu i =
      if i > 0 then
        aux (f (get_int t i) (Obj.magic (get t (succ i))) :: accu) (i - 2)
      else accu
    in
    aux [] (get_len t - 2)
...
let create_default def a =
  ...
  let rec aux i = function
    | [] -> if i == max_int then [] else [ (succ i, def) ]
    | ((i1, _) as c) :: rest ->
...
let rec find_aux t (i : int) low high =
  if low >= high then low
    ...
    if i < get_int t m then find_aux t i low (m - 2) else find_aux t i m high

let find (t : 'a t) i : 'a =
  if t == empty then raise Not_found;
  let j = find_aux t i 1 (get_len t - 2) in
  if get_int t j == i then Obj.magic (get t (succ j)) else raise Not_found

let find_default t def i =
  if t == empty then def
  else
    let j = find_aux t i 1 (get_len t - 2) in
    if get_int t j == i then Obj.magic (get t (succ j)) else def
    ...
    let rec aux i i1 (l1 : int) i2 l2 =
      if l1 == l2 then (
        ...
        if i1 = n1 then (
          ...
          let l1 = get_int t1 i1
          and l2 = get_int t2 i2 in
          let i1 = i1 + 2
          and i2 = i2 + 2 in
          aux i i1 l1 i2 l2)
      else if l1 < l2 then (
...
let cardinal t = if t == empty then 0 else pred (get_len t) lsr 1
  ...
  if t == empty then empty
    ...
    let rec aux i =
      if i = 0 then t'
        ...
        set t' i (Obj.magic (f (Obj.magic (get t i))));
        aux (i - 2))
    in
    aux (pred n)
      ...
      let rec aux i =
        if i < 0 then 0
          ...
          if l1 < l2 then -1
            ...
            let x1 = Obj.magic (get t1 (succ i))
            and x2 = Obj.magic (get t2 (succ i)) in
            let c = f x1 x2 in
  ...
  if t == empty then 1
    ...
    let rec aux accu i =
      if i < 0 then accu
        ...
        aux
          ((accu * 65537) + (257 * f (Obj.magic (get t (succ i)))) + get_int t i)
          (i - 2)
    in
    aux 1 (get_len t - 2)
  ...
  if t == empty then ()
    ...
    let rec aux i =
      if i < 0 then ()
        ...
        f (get_int t i) (Obj.magic (get t (succ i)));
        aux (i - 2)
      end
    in
    aux (get_len t - 2)
