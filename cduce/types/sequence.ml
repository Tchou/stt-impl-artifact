...
let nil_type = Types.atom (AtomSet.atom nil_atom)
let nil_node = Types.cons nil_type
let nil_cst = Types.Atom nil_atom

let decompose t =
  (AtomSet.contains nil_atom (Types.Atom.get t), Types.Product.get t)
...
let plus t =
  ...
  Types.times t (star_node t)

let string = star Types.Char.any
let option t = Types.cup (Types.times t nil_node) nil_type
...
let map_tree cstr f seq =
  ...
  and descr_aux cstr t v =
    let cstr0 = if !precise then Types.Product.normal ~kind:`XML cstr else [] in
    let cstr_tag = if !precise then Types.Product.pi1 cstr0 else Types.any in
    ...
    let f2 cstr_attr cstr1 (attr, child) =
      ...
      in
      V.times (V.ty attr) (aux cstr_sub child)
    in
    let f1 (tag, x) =
      let cstr1 =
        if !precise then (
...
let _map_tree_mono domain seq =
  ...
  ( !inp,
    !ts,
    fun fts ->
      List.iter2 (fun t (result, v) -> V.define result (aux_concat t v)) fts !vs;
      solve r )
...
let char_latin1 = Types.char CharSet.(mk_classes [ (V.mk_int 0, V.mk_int 255) ])
let string_latin1 = star char_latin1

let any_xtransformable =
  Types.descr
    (recurs (fun n ->
         let elt = Types.xml any_node (Types.cons (Types.times any_node n)) in
         let non_elt = Types.neg (Types.xml any_node any_node) in
