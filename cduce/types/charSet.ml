module V = struct
  ...
  let print_in_string ppf = function
    ...
    | c ->
        if c < 32 || c >= 128 then Format.fprintf ppf "\\%i;" c
        else Format.fprintf ppf "%c" (Char.chr c)

  let print ppf c = Format.fprintf ppf "'%a'" print_in_string c
end
...
let test = function
  | [] -> Tset.Empty
  | (inf, sup) :: [] when inf == 0 && sup == V.max_char -> Tset.Full
  | _ -> Tset.Unknown

let char_class a b = if a <= b then [ (a, b) ] else empty
let atom a = [ (a, a) ]

let rec add l ((a, b) as i) =
  match l with
  | [] -> [ i ]
  | (a1, _) :: _ as l when b < a1 - 1 -> i :: l
  | ((_, b1) as i') :: l' when a > b1 + 1 -> i' :: add l' i
  | (a1, b1) :: l' -> add l' (min a a1, max b b1)
...
let rec disjoint (a : t) b =
  ...
  | (xa, ya) :: a', (xb, yb) :: b' ->
      if xa = xb then false
      else if xa < xb then ya < xb && disjoint a' b
...
let print =
  List.map (fun (a, b) ->
      if a = b then fun ppf -> V.print ppf a
      else fun ppf ->
        if a = 0 && b = max_char then Format.fprintf ppf "Char"
        else Format.fprintf ppf "%a--%a" V.print a V.print b)
...
let mk_map l =
  let m =
    List.fold_left
      (fun accu (i, x) ->
        List.fold_left (fun accu (_, b) -> (b, x) :: accu) accu i)
      [] l
  in
  let m =
    List.sort
