...
let error s = Cduce_error.(raise_err Schema_common_XSD_validation_error s)
...
let particle_model min max mg =
  particle min max (Model mg) (first_of_model_group mg)
    (nullable_of_model_group mg)

let check_force v =
  try Lazy.force v with
  | Lazy.Undefined -> error "Cyclic type definition"
...
let bool_attr attr n =
  ...
  | Some v -> (
      ...
      | "0" ->
          false
      | s -> error ("Invalid boolean value (" ^ s ^ ") for attribute " ^ attr))
...
let split s = split_spaces s

(*let unqualify s = snd (Ns.split_qname s)*)

let hashtbl_deref tbl = QTable.fold (fun _ v acc -> check_force v :: acc) tbl []
let hashtbl_values tbl = QTable.fold (fun _ v acc -> v :: acc) tbl []

let parse_facets base n =
  let validate_base_type v = Schema_validator.validate_simple_type base v in
  let parse_nonneg n =
  ...
  let aux facets n tag =
    ...
    | "xsd:whiteSpace" ->
        ...
        let k =
          ...
          | "preserve" -> `Preserve
          | "replace" -> `Replace
          | _ -> error "Unknown value for whiteSpace facet"
        in
...
let register_builtins typs =
  Schema_builtin.iter (fun name b ->
      QTable.replace typs name (lazy (Simple (Schema_builtin.simple_type b))));
...
let schema_of_uri uri =
  ...
  let resolve k t1 t2 f qname =
    try QTable.find t1 qname with
    | Not_found ->
        let node =
          try QTable.find t2 qname with
          | Not_found ->
              error
                ("Can't find declaration for " ^ k ^ " "
  ...
  let rec parse_uri uri =
    ...
    let attributeFormDefault =
      _is_attr "attributeFormDefault" root "qualified"
    in
    let elementFormDefault = _is_attr "elementFormDefault" root "qualified" in
    ...
    let rec parse_root uri root =
      ...
      let rec resolve_typ qname : Schema_types.type_definition lazy_t =
        try QTable.find typs qname with
        | Not_found -> error ("Cannot find type " ^ Ns.QName.to_string qname)
      ...
      and resolve_model_group qname =
        resolve "model group" model_groups model_group_elems
          parse_model_group_def qname
      ...
      and find_member_types n =
        let members1 =
          ...
          | Some v ->
              List.map
                (fun x -> resolve_simple_typ (_resolve_qname n x))
                (split v)
          | None -> []
        in
        let members2 =
      ...
      and parse_elt_value_constraint (type_def : type_ref) n =
        let validate_value v =
          ...
          | Simple st
          | Complex { ct_content = CT_simple st } ->
      ...
      and parse_att_decl global n =
        ...
        let ns =
          if global then targetNamespace
          else
      ...
      and parse_attribute_uses n =
        let uses1 =
          ...
          ( List.map parse_attribute_use (_elems "xsd:attribute" n),
            match _may_elem "xsd:anyAttribute" n with
            | Some _ -> true
            | _ -> false )
        in
      ...
      and get_derivation content =
        let derivation, derivation_type =
          ...
          | None -> (
              ...
              | Some v -> (v, `Extension)
              | None -> error "No extension element found")
        in
      ...
      and parse_complex_type n = lazy (parse_complex_type_def n)
      ...
      and parse_elt_decl global n =
        ...
        element (AtomSet.V.mk (ns, local)) type_def value_constr nillable
      ...
      and parse_particle n =
        ...
        let elt e n = particle min max (Elt e) (AtomSet.atom n) (min = 0) in
        match _tag n with
        | "xsd:element" -> (
            match _may_qname_attr "ref" n with
            | Some ref -> elt (resolve_elt ref) (AtomSet.V.mk ref)
            | None ->
                let decl = parse_elt_decl false n in
                elt decl decl.elt_name)
        | "xsd:group" ->
            model (resolve_model_group (_qname_attr "ref" n)).mg_def
        | "xsd:all"
        | "xsd:sequence"
        | "xsd:choice" ->
      ...
      and parse_wildcard_process n =
        match _may_attr "processContents" n with
      ...
      and parse_model_group n =
        match _tag n with
        | "xsd:all" -> All (List.map parse_particle (_elems "xsd:element" n))
        | "xsd:sequence" ->
            Sequence (List.map parse_particle (find_particles n))
        | "xsd:choice" -> Choice (List.map parse_particle (find_particles n))
        | _ -> assert false
      ...
      and parse_model_group_def n =
        ...
        let model_group_node =
          match
            first n _may_elem [ "xsd:all"; "xsd:choice"; "xsd:sequence" ]
          with
          | Some m -> m
          | None -> error "No model group"
        in
      ...
      let check_redef n table kind =
        ...
        if QTable.mem table name then
          error ("Redefinition of " ^ kind ^ " " ^ Ns.QName.to_string name)
        else name
      in
      ...
      let rec register n = function
        ...
        | ("xsd:simpleType" | "xsd:complexType") as s ->
            ...
            let l =
              if s = "xsd:simpleType" then lazy (Simple (parse_simple_type n))
              else lazy (parse_complex_type_def n)
            in
        ...
        | "xsd:include" ->
            ...
            print_endline ("Include " ^ uri);
            flush stdout;
            parse_root uri (node_of_uri uri)
        | "xsd:import" -> (
            let i uri =
              print_endline ("Import " ^ uri);
              flush stdout;
              ignore (parse_uri uri)
            in
            match _may_attr "schemaLocation" n with
            | None -> (
                match _may_attr "namespace" n with
                | Some ns
                  when Utf8.get_str ns = "http://www.w3.org/XML/1998/namespace"
                  ->
                ...
                | Some ns ->
                    ...
                    i (Utf8.get_str ns)
                | _ -> print_endline "Import ignored, no schemaLocation")
            | Some local -> i (Url.local uri (Utf8.get_str local)))
