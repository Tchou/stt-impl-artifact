...
let nil_type = mk_type Types.Sequence.nil_type
...
let mk_seq_derecurs base facets =
  let min, max =
    ...
    | _ -> (1, Some 1)
  in
  Types.Sequence.repet min max base
...
and wildcard w = mk_type (Builtin_defs.any_xml_with_tag w.wild_first)
...
and get_complex ct =
  try Hashtbl.find complex_memo ct.ct_uid with
  | Not_found ->
...
and complex nil ct =
  ...
  if nil then
    ...
    mk_or c (mk_type (Types.times (Types.cons attrs) Types.Sequence.nil_node))
  else c

and content = function
  | CT_empty -> mk_type Types.Sequence.nil_type
...
and elt_decl elt =
  let atom_type = mk_type Types.(atom (AtomSet.atom elt.elt_name)) in
  ...
  let content =
    ...
    | Some (`Fixed (_, v)) ->
        mk_and content
          (mk_type
             (Types.times (Types.cons Types.any)
...
and complex_type_def nil = function
  ...
  | Simple st ->
      ...
      let t =
        if nil then
          Types.cup nonnil
            Types.(times (cons xsi_nil_type) (cons Sequence.nil_type))
        else nonnil
      in
      mk_type t
...
let load_schema schema_name uri =
  ...
  let defs kind name cd_type v lst =
    List.iter
      (fun def ->
        ...
        env := Env.add name (cd_type, v def) !env)
      lst
  in
  let schema = Schema_parser.schema_of_uri uri in
  defs "attribute group"
    (fun ag -> ag.ag_name)
    attr_group
    (fun x -> VAttrGp x)
  ...
  defs "model group"
    ...
    (fun x -> VModelGp x)
    schema.model_groups;
  defs "type" name_of_type_definition type_def (fun x -> VType x) schema.types;
  defs "element"
    (fun e -> AtomSet.V.value e.elt_name)
    elt_decl
    (fun x -> VElem x)
    schema.elements;
