...
let foo_atom = Value.Atom (AtomSet.V.mk_ascii "foo")
let foo_event = E_char_data (Utf8.mk "")

let qtable_is_empty tbl =
  try
    QTable.iter (fun _ _ -> raise Exit) tbl;
    true
  with
  | Exit -> false
...
let push_str ctx s =
  {
    ctx with
    ctx_stream = ref (fun () -> Seq.Cons (E_char_data s, !(ctx.ctx_stream)));
  }
...
let error s = Cduce_error.(raise_err Schema_common_XSI_validation_error s)
...
let xml qname attrs content = Value.Xml (Value.Atom qname, attrs, content)

let peek_rseq rseq =
  match !rseq () with
  | Seq.Nil -> None
  | Seq.Cons (ev, k) ->
      (rseq := fun () -> Seq.Cons (ev, k));
      Some ev

let peek ctx =
  match peek_rseq ctx.ctx_stream with
...
let next_rseq rseq =
  ...
  | Seq.Cons (e, k) ->
      rseq := k;
      e
  | Seq.Nil -> Cduce_error.raise_err Cduce_error.Schema_validator_Failure ()

let next ctx =
  try next_rseq ctx.ctx_stream with
  | Cduce_error.(Error (_, (Schema_validator_Failure, _))) -> error "Unexpected end of stream"

let junk_rseq rseq = ignore (next_rseq rseq)
let junk ctx = junk_rseq ctx.ctx_stream
...
let get_string ctx =
  ...
  let rec aux () =
    ...
    | E_char_data s ->
        ...
        aux ()
    | E_start_tag _ -> error "XML element found in simple content"
    | _ -> ()
  in
  aux ();
...
let rec ignore_ws ctx =
  ...
  | E_char_data s when only_ws s ->
      junk ctx;
      ignore_ws ctx
  | E_char_data _ -> error "Unexpected char data in non-mixed content"
  | _ -> ()

let do_pcdata ctx = if ctx.ctx_mixed then copy_pcdata ctx else ignore_ws ctx
...
let check_nil ctx =
  ...
  | ev ->
      error
        (sprintf "Non-empty content with xsi:nil set : %s" (string_of_event ev))

let expect_start_tag ctx tag =
  match next ctx with
  | E_start_tag t when AtomSet.V.equal t tag -> ()
  | ev ->
      error
        (sprintf "Expected tag %s, found %s" (AtomSet.V.to_string tag)
           (string_of_event ev))

let expect_any_start_tag ctx =
  match next ctx with
  | E_start_tag t -> t
  | ev -> error (sprintf "Expected start tag, found %s" (string_of_event ev))
...
let rec tries funs arg =
  ...
  | f :: tl -> (
      try f arg with
      | Cduce_error.Error (_, (Schema_common_XSI_validation_error, _)) -> tries tl arg)

(*let space_RE = pcre_regexp " "*)
let split = split_spaces

(** {2 Facets validation} *)

module Schema_facets : sig
  ...
  let length v =
    let rec aux acc = function
      | Pair { fst = _; snd = rest; concat = false } -> aux (succ acc) rest
      | _ -> 0
    in
    aux 0 v

  let length_valid len value =
    if length value != len then Cduce_error.(raise_err Schema_validator_Facet_error "length")

  let minLength_valid min_len value =
    if length value < min_len then Cduce_error.(raise_err Schema_validator_Facet_error "minLength")

  let maxLength_valid max_len value =
    if length value > max_len then Cduce_error.(raise_err Schema_validator_Facet_error "maxLength")

  let enumeration_valid enum value =
    if not (List.exists (fun x -> Value.equal value x) enum) then
      Cduce_error.(raise_err Schema_validator_Facet_error "enumeration")

  let maxInclusive_valid max_inc value =
    if value |>| max_inc then Cduce_error.(raise_err Schema_validator_Facet_error "maxInclusive")

  let maxExclusive_valid max_exc value =
    if value |>=| max_exc then Cduce_error.(raise_err Schema_validator_Facet_error "maxExclusive")

  let minInclusive_valid min_inc value =
    if value |<| min_inc then Cduce_error.(raise_err Schema_validator_Facet_error "minInclusive")

  let minExclusive_valid min_exc value =
    if value |<=| min_exc then Cduce_error.(raise_err Schema_validator_Facet_error "minInclusive")
...
let rec validate_simple_type def s =
  ...
  | { st_name = Some name } when Schema_builtin.is name -> (
      let value =
        ...
        | _ -> s
      in
      try Schema_builtin.validate (Schema_builtin.get name) value with
      | Cduce_error.Error (_, (Schema_builtin_Error, name)) ->
          error (sprintf "%s isn't a valid %s" (Utf8.to_string value) name))
  | { st_variety = Atomic st; st_facets = facets } ->
...
let validate_wildcard ctx w =
  ...
  if not (AtomSet.contains qname w.wild_first) then
    error
      (sprintf "Tag %s is not accepted by the wildcard"
         (AtomSet.V.to_string qname));
  let attrs, content = validate_any_type ctx in
  expect_end_tag ctx;
...
let check_fixed fixed value =
  if not (Value.equal fixed value) then
    error
      (sprintf "Expected fixed value: %s; found %s" (string_of_value fixed)
...
let validate_attribute_uses attrs (attr_uses, anyattr) =
  ...
  List.iter (fun use -> QTable.add tbl use.attr_decl.attr_name use) attr_uses;
  let attribs = ref [] in
  List.iter
    (fun (qname, value) ->
      let value =
        ...
        | Not_found ->
            if anyattr then Value.string_utf8 value
              ...
              error
                (sprintf "Unexpected attribute: %s"
                   (Ns.Label.string_of_attr qname))
      in
      attribs := (qname, value) :: !attribs)
    attrs.attrs;
  if attrs.xsi_nil then attribs := (xsi_nil_label, Value.vtrue) :: !attribs;
  QTable.iter
    (fun qname at ->
      if at.attr_required then
        (* check for missing required attributes *)
        error
          (sprintf "Required attribute %s is missing"
             (Ns.Label.string_of_attr qname))
      else
        (* add default values *)
...
and validate_type ctx attrs = function
  ...
  | Simple st_def ->
      ...
      if attrs.xsi_nil then (
        check_nil ctx;
        (Value.vrecord [ (xsi_nil_label, Value.vtrue) ], Value.nil))
...
and validate_complex_type ctx attrs ct =
  let content =
    if attrs.xsi_nil then (
      check_nil ctx;
      Value.nil)
    else validate_content_type ctx ct.ct_content
  in
...
and validate_content_type ctx content_type =
  ...
  | CT_empty -> Value.nil
  | CT_simple st_def -> validate_simple_type st_def (get_string ctx)
...
and validate_particle ctx particle =
  let rec validate_once ~cont_ok ~cont_failure =
    do_pcdata ctx;
    match peek ctx with
    | E_start_tag qname when AtomSet.contains qname particle.part_first ->
        ...
        validate_once
          ...
          ~cont_failure:(fun event ->
            if particle.part_nullable then ()
              ...
              error (sprintf "Unexpected content: %s" (string_of_event event)))
  in
...
and validate_choice ctx particles =
  ...
  let tbl = AtomSet.mk_map (List.map (fun p -> (p.part_first, p)) particles) in
  do_pcdata ctx;
  try
    match peek ctx with
    | E_start_tag q -> validate_particle ctx (AtomSet.get_map q tbl)
    | _ -> raise Not_found
  with
  | Not_found -> error (sprintf "Cannot choose branch of choice group")

and validate_all_group ctx particles =
  let slots = List.map (fun p -> (p, ref None)) particles in
  let tbl =
    AtomSet.mk_map (List.map (fun (p, slot) -> (p.part_first, (p, slot))) slots)
  in

  let rec aux () =
    match peek ctx with
    | E_start_tag q -> (
        let p, slot = AtomSet.get_map q tbl in
        match !slot with
        | Some x -> ()
        | None ->
...
let ctx stream =
  { ctx_stream = ref stream; ctx_mixed = false; ctx_current = Value.Absent }
...
let get_str v =
  if not (is_str v) then
    error "Only string values could be validate against simple types";
  ...
  let stream =
    ...
    | Record _ ->
        List.to_seq
          ...
          @ [ foo_event ])
    | _ ->
        error "Only record values could be validated against attribute groups"
  in
...
let validate_model_group { mg_def = mg } value =
  if not (Value.is_seq value) then
    error "Only sequence values could be validated against model groups";
