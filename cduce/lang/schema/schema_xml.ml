...
let error s = Cduce_error.(raise_err Schema_xml_Error s)
...
let rec root =
  { tag = "***"; attrs = []; children = []; ns = Ns.empty_table; father = root }
...
let start_element_handler name att =
  ...
  let tag =
    if Ns.Uri.equal ns xsd then "xsd:" ^ Utf8.get_str local else "***"
  in
  current := { tag; attrs; children = []; ns = table; father = !current }
...
let _fold_elems n x f = List.fold_left (fun x n -> f x n n.tag) x n.children

let _filter_elems names n =
  List.filter (fun n -> List.mem n.tag names) n.children

let _may_elem name n =
  try Some (List.find (fun n -> n.tag = name) n.children) with
  | Not_found -> None

let _iter_elems n f = List.iter (fun n -> f n n.tag) n.children
