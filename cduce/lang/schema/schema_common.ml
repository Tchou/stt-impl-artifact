...
let rec normalize_white_space handling s =
  match handling with
  | `Preserve -> s
  | `Replace -> replace_space s
  | `Collapse ->
      let s' = replace_spaces (normalize_white_space `Replace s) in
      replace_margins s'

let first_of_wildcard_constraint f =
  let open Types in
  match f with
  | WAny -> AtomSet.any
  | WNot ns -> AtomSet.diff AtomSet.any (AtomSet.any_in_ns ns)
  | WOne l ->
      List.fold_left
        (fun acc ns -> AtomSet.cup acc (AtomSet.any_in_ns ns))
        AtomSet.empty l

let first_of_model_group f =
  let open Types in
  match f with
  | All particles
  | Choice particles ->
      List.fold_left
        (fun acc p -> AtomSet.cup acc p.part_first)
        AtomSet.empty particles
  | Sequence particles ->
      let rec aux = function
        | hd :: tl when hd.part_nullable -> AtomSet.cup hd.part_first (aux tl)
        | hd :: tl -> hd.part_first
        | [] -> AtomSet.empty
      in
      aux particles
...
let stream_of_value v =
  ...
  let f _ =
    match !stack with
    | Fully
        (( Value.Xml (Value.Atom atom, attrs, _)
         | Value.XmlNs (Value.Atom atom, attrs, _, _) ) as v)
      :: tl ->
        stack := Half v :: tl;
        let children = ref [] in
        (* TODO inefficient *)
        ...
        List.iter (* push attributes as events on the stack *)
          (fun (qname, v) ->
            push
              (Backlog (E_attribute (qname, fst (Value.get_string_utf8 v))))
              stack)
  ...
  let rec self () =
    match f () with
    | Some e -> Seq.Cons (e, self)
    | None -> Seq.Nil
  in
  self

let string_of_event = function
  | E_start_tag t -> sprintf "<%s>" (AtomSet.V.to_string t)
  | E_end_tag -> sprintf "</>"
...
let xsi_nil_atom = AtomSet.V.mk (Schema_xml.xsi, Utf8.mk "nil")
let xsi_nil_type = Types.(atom (AtomSet.atom xsi_nil_atom))
