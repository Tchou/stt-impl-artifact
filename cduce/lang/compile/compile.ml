...
let find x env =
  try Env.find x env.vars with
  | Not_found -> failwith ("Compile: cannot find " ^ Ident.to_string x)
...
let enter_global_cu cu env x =
  {
    ...
    vars =
      Env.add x
        (Ext { cu; index = env.global_size; value = Value.Absent })
        env.vars;
    global_size = env.global_size + 1;
  }
...
and compile_aux env = function
  ...
  | Typed.External (t, ext) -> (
      match env.cu, ext with
      | Some cu, ((`Ext index)|(`Builtin (_, index))) ->
          Var
            {
              loc = External { cu; index; value = Value.Absent };
              value = Value.Absent;
            }
      | None, `Builtin (s,_) ->  Var { loc = Builtin s; value = Value.Absent }
      | _ -> failwith "Cannot compile externals in the toplevel")
  | Typed.Op (op, _, args) ->
  ...
  let slots, nb_slots, fun_env =
    List.fold_left
      (fun (slots, nb_slots, fun_env) x ->
        ...
        | Dummy -> assert false)
      ([ Dummy ], 1, fun_env) (IdSet.get a.Typed.fun_fv)
  in
...
and compile_branch env br =
  let env = IdSet.fold enter_local env (Patterns.fv br.Typed.br_pat) in
...
let compile_let_decl env decl =
  ...
  let env = enter_globals env (Patterns.fv pat :> Id.t list) in
