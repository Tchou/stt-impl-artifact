...
let print_ret lhs ppf (code, ret, ar) =
  ...
  if Array.length ret <> 0 then Format.fprintf ppf "(%a)" (print_result lhs) ret
...
let gen_lhs prefix d code =
  ...
  for i = 0 to arity - 1 do
    r := Format.sprintf "%s%i" prefix i :: !r
  done;
  !r
...
let print_kind ppf actions =
  ...
  let print_basic (t, ret) =
    Format.fprintf ppf " | %a -> %a@\n" Types.Print.print t (print_ret []) ret
  in
  let print_prod2 lhs = function
    | Impossible -> assert false
    | Ignore r -> Format.fprintf ppf "%a\n" (print_ret lhs) r
    | TailCall d -> Format.fprintf ppf "%a v2@\n" rec_state d
    | Dispatch (d, branches) ->
        Format.fprintf ppf "@\n        match %a v2 with@\n" rec_state d;
        Array.iteri
          (fun code r ->
