...
let make_result pt d (code, _, _) =
  if d.fail_code == code then Cduce_error.(raise_err Explain_Failed pt);
  code
        ...
        (fun t ->
          if is_poly then
            ...
            | `Other ->
                Value.failwith'
                  (Format.asprintf
                     "Run-time inspection of polymorphic function  %a  against \
                      type %a"
                     Types.Print.print
                     (List.fold_left
                        (fun acc (s, t) ->
  ...
  | Some (RecLabel (l, r)) -> (
      ...
      try
        run_disp_record1 pt d l (succ n)
          ...
          fields r
      with
      | Not_found -> run_disp_record1 pt d l n Absent fields r)
...
let is_xml = function
  | (Xml _ | XmlNs _), _ -> true
  | _ -> false

let rec simplify = function
  | (Absent, _) :: l -> simplify l
  | x :: l -> (
      try [ x; List.find is_xml l ] with
      | Not_found -> [ x ])
  | [] -> assert false
...
let explain d v =
  try
    ...
    None
  with
  | Cduce_error.(Error (_, (Explain_Failed, p))) ->
    let p : (Value.t * string) list = p in Some p
