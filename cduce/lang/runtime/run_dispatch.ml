    ...
    for a = 0 to n - 1 do
      let x =
        ...
        | Recompose (n, m) ->
            pair
              (match n with
              | -1 -> Char (CharSet.V.mk_int (Utf8.get s i))
              | -2 -> nil
              | _ -> buf.(c - n))
              (match m with
...
let rec run_disp_basic v f = function
  ...
  | (t, r) :: rem ->
      if f t then make_result_basic v r else run_disp_basic v f rem
  | _ ->
...
and run_disp_kind actions v =
  ...
  | Pair { fst = v1; snd = v2; concat = false } ->
      run_disp_prod v v1 v2 actions.prod
  | Xml (v1, v2, v3)
  | XmlNs (v1, v2, v3, _) ->
  ...
  | Atom q -> make_result_basic v (AtomSet.get_map q actions.atoms)
  | Char c -> make_result_basic v (CharSet.get_map c actions.chars)
  | Integer i ->
      run_disp_basic v
  ...
  | Abstraction (None, _, _) ->
      run_disp_basic v
        (fun t ->
          match Types.Arrow.trivially_arrow t with
          | `Arrow -> true
          | `NotArrow -> false
          | `Other ->
              Value.failwith' "Run-time inspection of external abstraction")
        actions.basic
        ...
        (fun t ->
          if is_poly then
            ...
            | `Other ->
                Value.failwith'
                  (Format.asprintf
                     "Run-time inspection of polymorphic function @[%a@] \
                      against type @[%a@]"
                     Types.Print.print
                     (List.fold_left
                        (fun acc (s, t) ->
  ...
  | Abstract (abs, _) ->
      run_disp_basic v
        (fun t -> AbstractSet.contains abs (Types.get_abstract t))
        actions.basic
  | Absent ->
      run_disp_basic v (fun t -> Types.Record.has_absent t) actions.basic
  | Pair { concat = true; _ } as v -> run_disp_kind actions (Value.normalize v)
...
and run_disp_record n v fields = function
  ...
  | Some (RecLabel (l, d)) -> (
      ...
      try
        run_disp_record1 v (succ n) (Imap.find fields (Upool.int l)) fields d
      with
      | Not_found -> run_disp_record1 v n Absent fields d)
...
and run_disp_string_latin1 i j s q actions =
  if i == j then run_disp_kind actions q
    ...
    | Impossible -> assert false
    | TailCall d1 -> run_disp_string_latin1_char d1 (CharSet.V.mk_char s.[i])
    | Ignore d2 -> run_disp_string_latin1_2 i j s q d2
    | Dispatch (d1, b1) ->
        let code1 = run_disp_string_latin1_char d1 (CharSet.V.mk_char s.[i]) in
        run_disp_string_latin1_2 i j s q b1.(code1)

and run_disp_string_latin1_char d ch =
  match d.actions with
  | AIgnore r -> make_result_char ch r
  | AKind k -> make_result_char ch (CharSet.get_map ch k.chars)

and run_disp_string_latin1_2 i j s q = function
  | Impossible -> assert false
  | Ignore r -> make_result_string_latin1 i j s q r
...
and run_disp_string_latin1_loop i j s q d =
  let i = succ i in
  if i == j then run_dispatcher d q
  else
    match d.actions with
    | AIgnore r ->
        make_result_basic (Value.String_latin1 { i; j; str = s; tl = q }) r
...
and run_disp_string_utf8 i j s q actions =
  if Utf8.equal_index i j then run_disp_kind actions q
    ...
    | Impossible -> assert false
    | TailCall d1 ->
        run_disp_string_utf8_char d1 (CharSet.V.mk_int (Utf8.get s i))
...
and run_disp_string_utf8_2 i j s q = function
  | Impossible -> assert false
  | Ignore r -> make_result_string_utf8 i j s q r
...
and run_disp_string_utf8_loop i j s q d =
  let i = Utf8.advance s i in
  if Utf8.equal_index i j then run_dispatcher d q
  else
    match d.actions with
    | AIgnore r ->
        make_result_basic (Value.String_utf8 { i; j; str = s; tl = q }) r
