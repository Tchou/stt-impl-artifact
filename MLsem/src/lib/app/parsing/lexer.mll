...
and read_ext buf depth = parse
| newline {
  Buffer.add_string buf (Lexing.lexeme lexbuf);
  enter_newline lexbuf |> read_ext buf depth }
| ")%" {
  if depth = 0 then EXT (Buffer.contents buf)
  else begin
    Buffer.add_string buf (Lexing.lexeme lexbuf);
    read_ext buf (depth - 1) lexbuf
  end
}
| "%(" {
  Buffer.add_string buf (Lexing.lexeme lexbuf);
